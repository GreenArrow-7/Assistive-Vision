...
def test_av14_labels_produce_av14_names(tmp_path):
    ...
    out = tmp_path / "out"
    ps.prepare(src, out, 0.25, schema=ps.SCHEMA_AV, allow_sparse=True)
...
def test_auto_refuses_to_guess_av14_from_low_indices(tmp_path):
    ...
    the AV schema would put AV names on COCO-trained weights, so it must refuse."""
...
def test_av14_flag_rejects_out_of_range_labels(tmp_path):
    ...
    with pytest.raises(SystemExit, match="class index 62"):
        ps.prepare(src, tmp_path / "out", 0.25, schema=ps.SCHEMA_AV, allow_sparse=True)
...
def test_val_frac_lands_near_target_not_past_it(tmp_path):
    ...
    out = tmp_path / "out"
    ps.prepare(src, out, 0.20, schema=ps.SCHEMA_AV, allow_sparse=True)
...
def test_sparse_av14_split_is_refused(tmp_path):
    ...
    with pytest.raises(SystemExit, match=f"{len(AV_CLASSES) - 3} of {len(AV_CLASSES)} AV classes have NO boxes"):
        ps.prepare(src, tmp_path / "out", 0.25, schema=ps.SCHEMA_AV)
...
def test_keep_empty_retains_them_as_background(tmp_path):
    ...
    for lbl in sorted((src / "labels").glob("mall_*.txt"))[:4]:
        lbl.write_text("")
    out = tmp_path / "out"
    ps.prepare(src, out, 0.25, schema=ps.SCHEMA_AV, allow_sparse=True, keep_empty=True)
...
def test_extra_source_honours_its_own_split(tmp_path):
    ...
    out = tmp_path / "out"
    ps.prepare(src, out, 0.25, schema=ps.SCHEMA_AV, allow_sparse=True, extra=ex)
...
def test_extra_labels_count_towards_the_sparse_guard(tmp_path):
    ...
    src = make_src(tmp_path, {"mall": 8, "hospital": 8}, [0, 1, 2])
    ex = make_extra(tmp_path, 6, 2, [3, 4])   # leaves the last class empty
    with pytest.raises(SystemExit) as e:      # still sparse: one class short
        ps.prepare(src, tmp_path / "out", 0.25, schema=ps.SCHEMA_AV, extra=ex)
    msg = str(e.value)
    assert f"of {len(AV_CLASSES)} AV classes have NO boxes" in msg, msg
    for supplied in ("door", "stairs_up"):    # the ids the extra source carries
...
def test_balanced_classes_do_not_trip_the_starved_guard(tmp_path):
    ...
    ps.prepare(src, tmp_path / "out", 0.25, schema=ps.SCHEMA_AV)   # must not raise
