    ...
    with TestClient(main.app) as client:          # lifespan starts main._warmup
        ...
        for i, img in enumerate(frames):
            ...
            for k, v in match(preds, gts, a.iou).items():
                for f in ("tp", "fp", "fn"):
                    cls_counts[k][f] += v[f]
            # vocabulary-independent, so comparable across COCO and AV-6.
    ...
    if p:
        ...
        if d.get("names_map"):
            m += ["_Predictions with no entry in the name map are ignored rather "
                  ...
                  "cannot express most AV-6 classes, and that structural zero is "
...
def main(a):
    ...
    if names:
        ...
        if model_schema == gt_schema == detector.SCHEMA_COCO and not a.allow_selfeval:
            raise SystemExit(
                ...
                "Correct the labels to AV-6 (docs/TRAINING.md Step 2) or pass "
    ...
    # model against AV-6 labels would score its "car" (2) against "table" (2).
    ...
    if a.task in ("pipeline", "all"):
        ...
        hazards = AV_HAZARDS if gt_schema == detector.SCHEMA_AV else config.HAZARD_CLASSES
