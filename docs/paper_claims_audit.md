...
(`models/av_obstacle_candidate.pt`, YOLOv8n/640/12ep, mAP@50 0.293) exists and
is evaluated in `runs/eval/av7_local.md`. It is NOT deployed and NOT the paper
model: `person` recall regresses against COCO, so quoting it as the system's
accuracy would overstate what a user gets. The final YOLOv8s run is pending.
...
| A4 | "released **custom symbol dataset**" / *AV-Symbol-2k*, 10 classes, ~200 images/class | The five `sign_*` classes hold **zero** boxes. No symbol dataset exists | **UNBACKED — delete C5** |
...
images (2,722 boxes). Split as built on 2026-08-29: 1,726 train / 350 val
frames, 4,787 / 691 boxes. Re-run `scripts/prepare_split.py` and refresh
