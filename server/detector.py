...
def detect_schema(names) -> str:
    ...
    OBJECT_MODEL_CUSTOM was assumed to be the AV-7 fine-tune. A COCO-class
    ...
    raise ValueError(
        ...
        f"must expose either the {len(AV_CLASSES)} AV-7 classes or all COCO "
...
def detect_objects(frame_bgr):
    ...
    av7 = model._av_schema == SCHEMA_AV7
    for xyxy, conf, cls in zip(res.boxes.xyxy.cpu().numpy(),
                               res.boxes.conf.cpu().numpy(),
                               res.boxes.cls.cpu().numpy()):
        label = names[int(cls)]
        # COCO base model: filter to curated classes; AV-7 model: keep all
        if not av7 and label not in config.OBJECT_CLASSES:
            continue
        ...
        if float(conf) < config.CLASS_CONF.get(label, config.OBJ_CONF):
            continue
        spoken = AV_SPOKEN.get(label, label) if av7 else label
        item = {
            "label": spoken,
            "raw": label,
        ...
        hazard = (label in AV_HAZARDS) if av7 else (label in config.HAZARD_CLASSES)
        if hazard:
